...
- [x] **`필수`** SearchInput 옆에 버튼을 하나 배치하고, 이 버튼을 클릭할 시 `/catAPI/cats/random` 을 호출하여 화면에 뿌리는 기능을 추가합니다. 버튼의 이름은 마음대로 정합니다.
...
```
  ...
  const catAPI = {
    fetchGif: keyword => {
      return request(`${API_ENDPOINT}/catAPI/gif/search?q=${keyword}`);
    },
    fetchGifAll: () => {
      return request(`${API_ENDPOINT}/catAPI/gif/all`);
    }
  };
```
...
- [DohwanKim/programmers-frontend-devMatching-2020-3](https://github.com/DohwanKim/programmers-frontend-devMatching-2020-3)
- [woohyeonjo/ilovecat-javascript](https://github.com/woohyeonjo/ilovecat-javascript)
